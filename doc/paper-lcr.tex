...
from the critical density and concentration parameter:
\be
...
\flag{kappa smooth correction.}

Thus the total $\kappax$ for a line of sight is given by 
\be
\kappax = \left(\Sum_{halos} \kappa_{halos}\right)-\kappa{smooth}.
\ee
...
it is possible to test the validity of using a truncated NFW halo model to estimate 
the external convergence along any given line of sight. 

For every halo and subhalo in the catalogue, we estimate a concentration according to
the best fit relation of \citet{neto2007}:
\be
c_{200} = 4.67 (M_{200}/10^{14} h^{-1} \Msun)^{-0.11}
\ee

Lines of sight are then chosen from random positions in the catalogue, and for each line of
sight we reconstruct the convergences from every halo within 5 arcminutes and compare the reconstructed
kappas with those from the ray tracing results of Hilbert et al. We do this for a range of
truncations, as a function of both the virial and NFW scale radius. The scatter is 
shown in Figure \ref{fig:ScattervsTruncation}. It is clear from Figure \ref{fig:ScattervsTruncation} 
that as long as the truncation is greater than the virial radius, the choice of truncation
radius does not broadly affect the scatter on $\kappah$-$kapparec$; from now on we choose 
to truncate the halos at three times their virial radius.
