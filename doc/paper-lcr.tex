...
In the case of several mass sheets at redshifts $\{z_p\}$, with surface mass densities $\{\Sigma_p\}$, the effective convergence is given by the \flag{weighted?} sum :
\be \label{eq:kappasum}
\kappax=\frac{4\pi G}{c^2}\sum_p\frac{\Sigma_p(D_p\vec\theta)D_p D_{p{\rm s}}}{D_{\rm s}}.
\ee
