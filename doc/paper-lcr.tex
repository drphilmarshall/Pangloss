...
\MS mock catalogues and their associated $\kappa$ values to construct
$\Pr(\kappa|\data)$. For the most over-dense lines of sight the
...
lens modeling, then the time delay distance -- inferred assuming $\kappax
...
We can hence estimate the true distance {\it only} if we have with additional
...
$\pr(\kappa|\data)$ in $10^{5}$ reconstructed lines of sight produces a
...
$\kappah$ is larger than in the spectrocopic case and this
...
$\kappa$ is reconstructed accurately. The reconstruction must not be biased. 
...
$\kappatrue$ from $\pr(\kappa|\data)$ we have an estimator of zero, $\pr(\kappa-\kappatrue|\data)$.
 Since the $\pr(\kappa-\kappatrue|\data)$ for
...
$\kappa$ lines of sight. With targeted spectroscopic follow-up of the selected lines of sight
...
select the best lines of sight for cosmology -- and doing so greatly increases
 the precision of the cosmological constraints per system, with no degredation of the accuracy.
Since lenses are typically found in locally overdense environments, it is unlikely that 
...
For the most overdense lines of sight,
...
Spectroscopic coverage will definately be useful for extracting cosmology from
overdense lines of sight.
...
$\kappa$ estimates. This will be useful
...
high $\kappa$ lines of sight, but is of less use for low $\kappa$ lines of
sight.
