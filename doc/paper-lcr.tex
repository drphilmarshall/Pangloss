...
We test the potential size of this systematic error by using the \citet{BehrooziEtal2010} relation to generate the stellar masses of our calibration lines of sight, but the Stellar Mass--Halo Mass relation of \citet{MosterEtal2010} for the 'real' lines of sight that are reconstructed. 
...
completely remove the dependence on simulation. \new{Throughout this work, we assumed
that the stellar mass--halo mass relation used for the reconstruction is the stellar mass--halo mass
relation followed by the real universe. We discarded the stellar masses from the Millenium Catalogue
due to a large number of excessively stripped satellite galaxies, however if we use these stellar masses
there is no systematic change in the ensemble, but reconstructed $\kappax$ differs by $\sim 0.015$ for individual lines of sight. The effect is smaller ($\sim 0.008$)
for the lowest $\kappax$ lines of sight, and larger ($\sim 0.02$) for high shear lines of sight. This 
change of stellar masses is extreme but demonstrates that using the wrong stellar mass--halo mass relation
can significantly degrade the quality of inference.} Future work should include
