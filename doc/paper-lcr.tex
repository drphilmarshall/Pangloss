...
$\mathcal{P}_N$ with a Gaussian of width $0.03/\sqrt{N}$ is a reasonable
...
lenses;  approximating strong-lens modelling uncertainties as an unbiased
Gaussian with width 4\% on time-delay distance a 0.8\% bias would be
significant for $\sim$20 lenses. \flag{We said 3\% earlier! Can you check for
consistency please? And we only need to explain the Gaussian convolution
once.} This systematic error is mitigated if only the lines of sight with both
...
\citep[\eg][]{MaxBCG} in tandem with BCG stellar mass. \tom{Meanwhile,
...
place. \flag{How much more precise are we than Greene et al?}
