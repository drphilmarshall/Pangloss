...
masses, \phil{we find that the reconstruction provides an improvement in
precision of 50\% over the global $\pr(\kappax)$.} 
%
...
%
\phil{Conversely, we show that somehow preferentially selecting lines of sight
with high external shear could, with the reconstruction model presented here,
induce biases of up to 1\% in time delay distance.}  We suggest areas for the
...
to rescale the time-delays. As such, it is necessary to include $\kappax$ in
...
of sight. \phil{However, in this paper we restrict ourselves to reconstructing
the line of sight portion of the mass distribution in any given field, which
allows us to work in the weak lensing regime. Treatment of sightlines
containing strong lensing events, and the impact of physically-associated mass
distributions on both the strong lensing and stellar dynamical effects are
...
systematic errors. \phil{On the other hand, making use of all the information
...
\phil{(In this case, $\kappah^{\rm med} = \kappah$, since 
$\pr(\kappah|\data)$ is a delta function)}.
% In this case, the PDF $\pr(\kappah|\data)$ is a delta function: we can read
...
\flag{Can you plot a $y=x$ line to show the correction being made, please?}}
\label{fig:jointkh-k}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
$\pr(\kappax|\data)$ grows with the expectation value of $\kappax$
...
\propogate into $\kappax$ uncertainties. 
The median reconstruction width in
this idealised situations is 0.01; some 22\% of lines of sight would have a
reconstruction width less than 0.01. }
...
having complete spectroscopic coverage (\Fref{fig:reconwidths}). High
$\kappax$ lines of sight benefit the most from additional spectral coverage,
since these have the most uncertain $\kappah$. \flag{This is not what
\Fref{fig:widthsvsH} shows at all! Does the $\kappaxtrue$ axis not extend far
enough?} Consistent with our previous findings, the lowest $\kappax$ lines of
...
\flag{Modify this as advised by Sherry -- introduce model for other
uncertainties in lenses, and explain number of lenses to see error argument.}
...
20 lines of sight. \phil{This result is really a sanity check, since for
random lines of sight the bias as defined here should be zero by
construction}. If these lines of sight hosted time delay lenses, the bias
number would correspond to a systematic error of 0.5\% in time delay distance.
\flag{What is this number? In the first sentence the bias was given as zero!}
Since the statistical distance uncertainties due to lens modelling and time
delay estimation are likely to be at the 3-4\% level \citep{SuyuEtal2010}, 
only a sample containing 200 or more lenses would be limited by the residual
bias following external convergence reconstruction.
\flag{$0.035/\sqrt{200} \neq 0.005$ so what numbers are these? We should
outline the lens number calculation in the preamble above.}
...
$\kappax$ lines of sight. With targeted spectroscopic follow-up of the
selected lines of sight there is only a slight 
improvement compared to photometry
alone, since the best constrained lines of sight are also 
the most empty, and it is
the high-$\kappax$ lines of sight that benefit the most from spectroscopy.
\flag{This was not shown conclusiveley earlier. Need to tighten this up.}
...
would be systematically underestimated at the $\sim$0.008 level (corresponding to a
0.8\% systematic error in distance).} 
...
and Greene et al (submitted). The reconstruction method presented here 
can be seen as the
...
\flag{How much more precise are we than Greene et al?}

\flag{Paragraph outlining future work on lens lines of sight (eg including
local environment). Why wasn't this done here?}
...
completely remove the dependence on simulation.
...
sight results in a sample with low bias, 0.1\% in terms of the time delay
distance, and a statistical uncertainty due to line of sight mass structure
that is half that of the parent sample. \flag{This is the first time this
number (0.1\%) has appeared!} Neglecting other sources of uncertainty, the
residual systematic error would be equal to the overall statistical
uncertainty in a sample of around 200 lenses. \flag{Needs checking.}
