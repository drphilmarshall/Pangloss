...
discrepancies.
%
\citet{KarpenkaEtal2012} used a halo model to predict the magnification due to
galaxies along the line of sight to type Ia supernovae, and inferred the
parameters of the scaling relations between light and mass in these galaxies
...
NFW profiles. Both groups detect lensing effects in the supernova data.
...
% While it is rare for three galaxies
% to line up well enough for both of the background sources to be strongly
% lensed \citep{GavazziEtal2008,CollettEtal2012a}, 
...
arcminutes, this fraction is \todo{XX\%}{Tom?}.
...
% \todo{Tom}{Explain why we re-made the stellar masses!}. 
...
%\todo{Tom}{Check that you mean $\kappah$ and not $\kappa$, and potentially change the graph!}}
\label{fig:widthsvsH}
\end{figure}
...
necessary. \citet{KarpenkaEtal2012} find that their supernova data prefer, in
the context of their simple halo model, a truncated isothermal profile for the
galaxy mass distributions. Weak galaxy-galaxy and cluster-galaxy 
lensing studies
\citep[\eg][]{GavazziEtal2006,JohnsonEtal2007} also suggest an approximately
...
should be taken into account by our model. Further iterations on the analysis
presented here should include the stellar mass distributions that cause the
isothermal density profiles in the centres of galaxies, although we do not
expect this to have a big impact on the model. Likewise,  halo
ellipticity may have some small effect on the predicted convergence. 
...
are discussed above. The latter tie is more difficult, since breaking it means
...
\citet{KarpenkaEtal2012} -- how much this dilutes the cosmographic precision
is to be investigated.  Including more information in this analysis is key:
weak lensing shears will be available over the whole Euclid and LSST survey
areas: fitting these with the halo models described here in order to constrain
the hyper-parameters of the model is an interesting
possibility.
...
$\kappa$ estimates. This procedure results in a sample with low bias as well
as high precision.
...
and following up only the systems with the most precise $\kappa$ estimates,
...
it is likely that lens modelling induces uncertainties at the $\sim$3 percent
level, which far exceeds the typical $\kappa$ uncertainties after applying our
reconstruction \proceedure. By using our method to reconstruct lines of sight
and select which lense are followed-up, the uncertainties due to line of sight
