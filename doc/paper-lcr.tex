...
\phil{(In this case, $\pr(\kappah|\data)$ is almost delta function since the only uncertainty is halo
...
\tom{With photometric data alone, $\kappah^{\rm med}$ can shift by $\sim$0.01 relative to $\kappah^{\rm med}$
given spectroscopic coverage. The $\kappax$ contribution of voids cannot change with data quality;
the shift in $\kappah^{\rm med}$ is due to the asymmetric propogation of stellar mass uncertainties
into $\kappax$ uncertainties. As stellar mass uncertainty increases $\kappah^{\rm med}$ is pushed higher. 
To correctly calibrate  $\pr(\kappax|\kappah^{\rm med},\data)$ one {\it must} include the data quality used to generate $\kappah^{\rm med}$.}
...
having complete spectroscopic coverage (\Fref{fig:reconwidths}).
...
significantly less than 0.01. \tom{External convergence is not the only
source of uncertainty in measuring time-delay distance.
...
convolving $\mathcal{P}_N$ with a Gaussian of width $0.03/\sqrt{N}$ is a
reasonable approximation for the 
likely final uncertainty on time-delay distance.}
...
20 lines of sight. \tom{This result is a sanity check: if we had chosen lines of
sight with identical $\kappah^{\rm med}$ rather than $\kappah^{\rm med}\pm0.003$ randomly
selected lines of sight would have zero bias by construction}. If lines of sight hosting time delay lenses
...
predict $\kappa_{\rm h,obs}^{\rm med}$ for use in their place. \flag{How much more precise are we than Greene et al?}
...
\tom{ Future work should focus on two areas:
reducing the dependance on simulations; and,
improving the modeling of dark structures and voids. The analysis should be repeated with different 
simulations and differrent stellar-mass to halo-mass relations, cross-checking that 
using the wrong simulation simulation or relation does not induce a systematic error.
Improving the modelling of of voids could be done using the void models of \citet{voids2}
or \citet{voids1} combined with a void finder such as in \citet{voids3}.
Combining the halo model with a void model may allow a direct test of the 
halo approximation; using real photometric data one could reconstruct lines of sight
and compare against measured weak lensing shears. This would ensure that the halo model reproduces the
contents of the real universe, rather than a particular simulation.}
...
distance. \tom{For these lines of sight the reconstructed $\pr(\kappax)$ induce 
a statistical uncertainty of $\sim$1\% on individual time-delay distances}
