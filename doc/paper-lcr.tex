...
\ee
We take $\sigma$ as 0.01- this is an overestimate of the uncertainty on $\tilde\kappa_{\rm halos}$, but ensures that the full distibution of void corrections is sampled, despite the finite number of calibration sightlines.
...
of $\kappax$. We will investigate two main sources of error: inferring halo mass given observed stellar mass and the combination of stellar mass errorsand placing halos at the wrong redshift due to photometric redshift error.
...
For each investigated source of error, we reconstruct 10000 lines of sight and each of the reconstructed $\pr(\sumkappah)$ is summarized by their mean $\sumkappah$, and their half-width containing $68\%$ of the probability, $\sigt$.
Reconstructing the lines of sight with perfect knowledge of the redshift but an uncertain stellar mass, we find that $\sigt$ grows with $\sumkappah$; this is not surprising since the low $\sumkappah$ lines of sight are relatively empty and so there are few opportunities for uncertainties in the halo masses to propogate into $\sumkappah$ uncertainties. The means and widths of these reconstructions are summarized in Figure \ref{fig:width1}, before and after applying the void correction and global prior ${\rm U}(\kappax)$. Before applying the void correction, the width of the reconstructions are roughly proportional to the mean of $\sumkappah$; they are well fit by a straight line given by 
\be
\sigt=-0.003+0.25 \langle\sumkappah\rangle.
\label{eq:Mstarfit}
\ee
After applying the void correction the expected bias is $-1.7\times 10^{-4}$ and the standard deviation for the bias is given by 0.0231; this is 1.2 times worse than the reconstruction given perfect knowledge of the halo masses and redshifts but still 1.5 times better than using the global P$^{\rm global}(\kappax)$; this is the reconstruction error that could be expected from a complete spectroscopic survey of the field. A complete spetroscopic survey is unrealistic, but Equation \ref{eq:Mstarfit2} represents a lower limit on the reconstruction error. For a randomly selected line of sight, the expectation value for the uncertainty on the reconstruction is 0.017 and the expectation value for the bias on E$(\kapparec)-\kapparay$ is 0.004.

\begin{figure}
\includegraphics[width=\columnwidth]{kappaspectro.eps}
\caption[radius cut]{Means and widths of $\kapparec$ for a complete spectroscopic survery of all halos down $to i=26$ and within 5 arcminutes of the line of sight. Results for 10000 lines of sight are shown. Red is the width and mean after summing up all the reconstructed halo contributions. Blue is the width of mean after correcting for voids. Green is the result after weighting the PDFs so that the universal kappa distribution is recovered}
\label{fig:width1}
\end{figure}

\begin{figure}
\includegraphics[width=\columnwidth]{kappasmall.eps}
\caption[radius cut]{As figure \ref{width1}, but for a photometric reconstruction of all halos down $to i=24$ and within 2 arcminutes of the line of sight}
\label{fig:width2}
\end{figure}

For fixed halo masses at unknown redshifts, we follow the same proceedure as the previous paragraph. We again find that $\sigt$ grows with $\kapparec$, but the error is small compared to the typical uncertainties for the reconstruction of halos at fixed redshifts but with uncertain masses. The fit is gven by
\be
\sigt=-0.002+0.15 \langle\kapparec^{\rm halos}\rangle.
\label{eq:Mstarfit}
\ee
Whilst redshift errors are a subdominant uncertainty at fixed halo mass, the true halo mass of an astronomical object is an unknown. Inferring the stellar mass of a galaxy from its magnitude and colours requires an estimate of how far away the galaxy is; without a spectroscopic redshift the the infered stellar mass is less precise. One should expect that given uncertain redshift and stellar mass the $\kappax$ reconstruction will be significantly more uncertain than a quadrature sum of the individual uncertainties. In principle the photometric redshift is correlated with the infered stellar mass, however we do not model this since the redshift error produces a small effect on the recovered $\pr(\kappa)$ when compared to the effect of uncertain stellar masses. For a complete photometric survey down to $i=26$ our halo reconstructions are fit by the line
\be
\sigt=-0.003+0.47\langle\kapparec^{\rm halos}\rangle
\label{eq:bothfit}
\ee
With only photometric redshifts the uncertainty on $\kapparec^{\rm halos}$ is much larger than the spectrocopic case, and this propogates into a much larger bias after applying the void correction. The expectation for the bias is -0.008 which is statistically signifiant given 10000 cones and a standard deviation of 0.0235. The reason for this bias relates to the difficulty of reconstructing halo masses given only a photometric redsshift. There are many massive halos that are intermediate distances from the line of sight; they do not contribute significantly to the true $\kappax$, but without spectroscopic reshifts their highly uncertain stellar masses propogate into broad $\pr(\Mhalo)$ and into a small probability of large individual $\kappax$ contributions. Since there are typically several hundred such objects within 5 arcminutes of the line of sight, their net contribution dominates the width of the photometrically generated $\pr \left(\sum_{\rm halos}\kappa_{\rm halo}\right)$, and artificially increases the mean of $\sumkappah$ despite the fact that they do not contribute much to the true $\kappax$. In equation \ref{eq:weighting} we assumed a symmetry for the weighting scheme used to form P($\kappax$) from $\pr(\kapparay,\tilde\kappa_{\rm halos})$. The assumption of symmetry is not valid since $\pr(\sumkappah)$ given perfect knowledge of the halo mass and redshift does not map linearly onto $\pr(\sumkappah)$ given only a photometric survey of the field. 


\subsection{Reconstructions of Smaller and Shallower Fields}
Reconstructions of real lines of sight may not have access to the deep, large calibration fields that we have used upto this point. We can perform the same reconstruction but for a more modest survey. Given the results of Section \ref{sec:knownMh+z} we predicted that a reconstruction of the field out to 2 arcminutes and a magnitude limit of $i$=24 would provide similar constraints to a reconstruction going out to 5 arcminutes and going down to $i$=26. For the purely photometric reconstruction the uncertainty on the halo reconstruction is given



Without an improved model of halos and/or reconstruction of voids, a field of 2 arcminute radius and depth of $i=24$ is sufficient to adequately reconstruct the external convergence.
...
%\section{Reconstructed Kappa as a tracer of Rayraced Kappa}
