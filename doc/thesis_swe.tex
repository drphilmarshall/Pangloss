...
\begin{align}
\varepsilon_{n_t}&=-\operatorname{Re}\left(\varepsilon e^{-2i\alpha}\right),\\
\varepsilon_{n_\times}&=-\operatorname{Im}\left(\varepsilon e^{-2i\alpha}\right).
\end{align}

\noindent Instead of using the autocorrelation and cross-correlation between these quantities, it turns out to be more convenient to define the combinations

\begin{align}\label{gg_corr_def}
\xi_\pm&=\left<\varepsilon_{i_t}\varepsilon_{j_t}\right>\pm\left<\varepsilon_{i_\times}\varepsilon_{j_\times}\right>\\
\xi_\times&=\left<\varepsilon_{i_t}\varepsilon_{j_\times}\right>
\end{align}

\noindent where $\theta$ is the position vector of the first galaxy and $\Delta\vec{\theta}$ is the angular separation vector between the pair of galaxies \cite{schneider}. Gravitational lensing shears ellipticities tangentially, so $\xi_\times(\Delta\theta)$ should vanish with enough galaxies. (\textcolor{red}{Why is $\xi_-$ expected to be zero?}), which leaves $\xi_+(\Delta\theta)$ as the desired probe of lensing mass.
...
% \begin{figure}
%     \centering
%     \includegraphics[width=0.475\textwidth]{figs-swe/thesis/kappamaps.png}
%     \captionsetup{justification=raggedright,singlelinecheck=false}
%     \caption{CAPTION}
%     \label{corr_fraction}
% \end{figure}
